...
import shutil
import os
from data_loader import split_dataset, ImgDataSet
import argparse
import tqdm
import numpy as np
...
def create_model(device, type ='vgg16'):
    if type == 'vgg16':
        ...
        model = UNet16(weights=True)
    elif type == 'resnet101':
        encoder_depth = 101
        num_classes = 1
        print('create resnet101 model')
        model = UNetResNet(encoder_depth=encoder_depth, num_classes=num_classes, weights=True)
    elif type == 'resnet34':
        encoder_depth = 34
        num_classes = 1
        print('create resnet34 model')
        model = UNetResNet(encoder_depth=encoder_depth, num_classes=num_classes, weights=True, is_deconv=True)
    else:
        assert False
    model.eval()
...
def display_batch_image(loader):
    def RGB2GRAY(mask):
        thresh = 0.5
    ...
    masks = torchvision.utils.make_grid(masks).numpy()
    masks = np.transpose(masks, (1,2,0))
    masks = RGB2GRAY(masks)
...
if __name__ == '__main__':
    ...
    parser.add_argument('-orig_json_path', type=str, required=True, help='input dataset jsonfile')
    parser.add_argument('-split_json_dir', type=str, required=True, help='save split_dataset jsonfile')
    parser.add_argument('-model_dir', type=str, required=True, help='output dataset directory')
    parser.add_argument('-model_type', type=str, required=True, choices=['vgg16', 'resnet101', 'resnet34'])

    args = parser.parse_args()
    os.makedirs(args.model_dir, exist_ok=True)

    orig_json_path = args.orig_json_path
    split_json_dir = args.split_json_dir
    split_set_json_filename = 'crack_dataset_split.json'
   
    with open(orig_json_path) as jsf:
        data_load = json.load(jsf)
        imPaths = data_load['imgpaths']

    print(f'total images = {len(imPaths)}')
    ...
    jsfList = [jsf for jsf in Path(split_json_dir).glob('crack_dataset_split.json')]
    split_set_json_filename = os.path.join(*[split_json_dir,split_set_json_filename])
    if jsfList == []:
        split_dataset(orig_json_path, 0.15, save_jsonfile=split_set_json_filename)
    
    train_dataset = ImgDataSet(split_set_json_filename, isTrain=True, img_transform=train_tfms, mask_transform=mask_tfms)
    valid_dataset = ImgDataSet(split_set_json_filename, isTrain=False, img_transform=train_tfms, mask_transform=mask_tfms)
